"""
FinBERT + LSTM Price Forecasting Pipeline (Paper-Aligned; 80/20 Split)
=======================================================================
Purpose & Scope
---------------
...
as requested.
It handles:
    - Loading and **schema resolution** for news and OHLCV prices data (flexible column names).
...
1. **Load & Resolve Schema**
   - Robust column resolver for: news `ticker|symbol|code|stock_symbol`, `published|time|datetime|date`, `title|headline|article_title`,
...
def resolve_schema(news_df: pd.DataFrame, prices_df: pd.DataFrame) -> Schema:
    news_ticker = _choose_first_present(news_df, ["ticker", "symbol", "code", "stock_symbol"])
    news_time = _choose_first_present(news_df,
                                      ["public_time", "published", "published_at", "time", "datetime", "date"])
    news_title = _choose_first_present(news_df, ["title", "headline", "article_title"])
    ...
    if price_date is None or price_close is None:
        missing = []
...
def main():
    ...
    start_date = pd.Timestamp(2015, 1, 1)   # Year, Month, Day
    end_date   = pd.Timestamp(2020, 12, 31) # Year, Month, Day
    ...
    news_df = pd.read_csv(
        ...
        parse_dates=["Date"],
        date_format="%Y-%m-%d"
    ).loc[(news_df.index > start_date) & (news_df.index < end_date)]
