...
def train_model(
    ...
    for ep in range(1, epochs + 1):
        ...
        if tr_pred_s_batches:
            ...
            tr_mse_price = mse(y_true, y_pred)
            tr_mae_price = mae(y_true, y_pred)
            tr_rmse_price = rmse(y_true, y_pred)
        else:
        ...
        if val_pred_s_batches:
            ...
            y_pred = y_scaler.inverse_transform(y_pred_s.reshape(-1, 1)).ravel()
            y_true = y_scaler.inverse_transform(y_true_s.reshape(-1, 1)).ravel()
            val_mse_price = mse(y_true, y_pred)
            val_mae_price = mae(y_true, y_pred)
            val_rmse_price = rmse(y_true, y_pred)
        else:
...
def main():
    ...
    news_df = load_and_filter_news(fnspid_path, kaggle_path, args.data_source, start_date, end_date, args.use_bodies)
    ...
    for ticker in tickers:
        ...
        df_supervised, feat_cols = build_supervised_for_ticker(df_joined, ticker=ticker, lookback=args.lookback)
